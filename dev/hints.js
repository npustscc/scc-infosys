...
const TC_HINTS = [
  ...
  { id: 4,  icon: 'autosave',   title: '表單自動備援',    desc: '表單打到一半不怕丟：晤談記錄、初談表、結案評估、轉銜學生初評、寫信等表單每 5 秒自動在本機暫存草稿，意外重新整理或系統強制更新後，重新開啟該表單即可還原。',
    tw: { t: '表單自動備份', d: '表單拍一半免驚無去：晤談紀錄、初談表、結案評估、轉銜初評、寫批信這寡表單，逐 5 秒仔會家己共草稿暫存佇本機，若拄著袂細膩重整抑是系統強制更新，才閣打開仝一張表單就會當復原。' },
    hk: { t: '表單自動備份', d: '表單打一半毋使驚無忒：晤談紀錄、初談表、結案評估、轉銜初評、寫信這兜表單，逐 5 秒自家會共草稿暫存在本機，若係堵到無細義重整抑係系統強制更新，再打開共一張表單就做得復原。' },
    en: { t: 'Auto form backup', d: 'Never lose a half-finished form: interview records, intake forms, closure assessments, transfer assessments, and compose-mail forms all auto-save a local draft every 5 seconds — reopen the same form after an accidental refresh or a forced update to restore it.' } },
