#!/usr/bin/env node
// smoke.mjs — VM 端 13 項 curl 冒煙（Node fetch 版；放在 server/ 目錄下執行）。
...
// 11. casesUpsert → Not implemented
r = await call({ action: 'casesUpsert', rootFolderId: ROOT, sessionToken: token });
check('11 casesUpsert → Not implemented', JSON.stringify(r).includes('Not implemented'), JSON.stringify(r));

// 12. logout 後舊 token 失效
...
r = await call({ action: 'ping', rootFolderId: ROOT, sessionToken: token });
check('12 logout 後舊 token → Session expired', r.data?.error === 'Session expired', JSON.stringify(r));
