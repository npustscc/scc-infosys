...
test('omsvFolderCreate：帶 parentId 建子資料夾 → list 含正確 parentId', () => {
  const db = openDb(':memory:');
  const owner = 'tree-roundtrip@x.com';
  const root = archive.omsvFolderCreate(db, owner, { name: '根層' });
  assert.equal(root.ok, true);
  const child = archive.omsvFolderCreate(db, owner, { name: '子層', parentId: root.id });
  assert.equal(child.ok, true);
  assert.equal(child.parentId, root.id);

  const list = archive.omsvFolderList(db, owner).folders;
  const rootRow = list.find((f) => f.id === root.id);
  const childRow = list.find((f) => f.id === child.id);
  assert.equal(rootRow.parentId, null);
  assert.equal(childRow.parentId, root.id);
});

test('omsvFolderCreate：深度上限——三層可、四層 omsv_folder_too_deep', () => {
  const db = openDb(':memory:');
  const owner = 'depth-test@x.com';
  assert.equal(archive.MAX_FOLDER_DEPTH, 3);
  const l1 = archive.omsvFolderCreate(db, owner, { name: 'L1' });
  const l2 = archive.omsvFolderCreate(db, owner, { name: 'L2', parentId: l1.id });
  const l3 = archive.omsvFolderCreate(db, owner, { name: 'L3', parentId: l2.id });
  assert.equal(l2.ok, true);
  assert.equal(l3.ok, true);
  const l4 = archive.omsvFolderCreate(db, owner, { name: 'L4', parentId: l3.id });
  assert.equal(l4.error, 'omsv_folder_too_deep');
});

test('omsvFolderCreate：parentId 指到別人的資料夾 → omsv_folder_not_found', () => {
  const db = openDb(':memory:');
  const a = archive.omsvFolderCreate(db, 'move-cross-a@x.com', { name: 'A 的資料夾' });
  const r = archive.omsvFolderCreate(db, 'move-cross-b@x.com', { name: 'B 的子資料夾', parentId: a.id });
  assert.equal(r.error, 'omsv_folder_not_found');
});

test('omsvFolderMove：基本移動——移入另一資料夾、再移回根層', () => {
  const db = openDb(':memory:');
  const owner = 'move-basic@x.com';
  const a = archive.omsvFolderCreate(db, owner, { name: 'A' });
  const b = archive.omsvFolderCreate(db, owner, { name: 'B' });
  const moved = archive.omsvFolderMove(db, owner, { folderId: b.id, parentId: a.id });
  assert.equal(moved.ok, true);
  assert.equal(archive.omsvFolderList(db, owner).folders.find((f) => f.id === b.id).parentId, a.id);

  const backToRoot = archive.omsvFolderMove(db, owner, { folderId: b.id, parentId: null });
  assert.equal(backToRoot.ok, true);
  assert.equal(archive.omsvFolderList(db, owner).folders.find((f) => f.id === b.id).parentId, null);
});

test('omsvFolderMove：移到自己 → omsv_folder_cycle；移到自己的孫子 → omsv_folder_cycle', () => {
  const db = openDb(':memory:');
  const owner = 'move-cycle@x.com';
  const a = archive.omsvFolderCreate(db, owner, { name: 'A' });
  const selfMove = archive.omsvFolderMove(db, owner, { folderId: a.id, parentId: a.id });
  assert.equal(selfMove.error, 'omsv_folder_cycle');

  const b = archive.omsvFolderCreate(db, owner, { name: 'B', parentId: a.id });
  const c = archive.omsvFolderCreate(db, owner, { name: 'C', parentId: b.id }); // A 的孫子
  const grandchildMove = archive.omsvFolderMove(db, owner, { folderId: a.id, parentId: c.id });
  assert.equal(grandchildMove.error, 'omsv_folder_cycle');
});

test('omsvFolderMove：移動後子樹過深（自身高度2 的樹移到第2層，2+2>3）→ omsv_folder_too_deep', () => {
  const db = openDb(':memory:');
  const owner = 'move-toodeep@x.com';
  // target：一個第 2 層的資料夾（深度=2）
  const targetRoot = archive.omsvFolderCreate(db, owner, { name: 'target-root' });
  const target = archive.omsvFolderCreate(db, owner, { name: 'target-l2', parentId: targetRoot.id });
  // moved：自身高度為 2 的樹（moved 底下還有一層子資料夾）
  const moved = archive.omsvFolderCreate(db, owner, { name: 'moved-root' });
  archive.omsvFolderCreate(db, owner, { name: 'moved-child', parentId: moved.id });

  const r = archive.omsvFolderMove(db, owner, { folderId: moved.id, parentId: target.id });
  assert.equal(r.error, 'omsv_folder_too_deep');
});

test('omsvFolderMove：folderId／parentId 指到別人的資料夾 → omsv_folder_not_found', () => {
  const db = openDb(':memory:');
  const a = archive.omsvFolderCreate(db, 'move-owner-a@x.com', { name: 'A' });
  const bOwnFolder = archive.omsvFolderCreate(db, 'move-owner-b@x.com', { name: 'B 自己的資料夾' });

  const moveOthersFolderId = archive.omsvFolderMove(db, 'move-owner-b@x.com', { folderId: a.id, parentId: null });
  assert.equal(moveOthersFolderId.error, 'omsv_folder_not_found');

  const moveIntoOthersParent = archive.omsvFolderMove(db, 'move-owner-b@x.com', { folderId: bOwnFolder.id, parentId: a.id });
  assert.equal(moveIntoOthersParent.error, 'omsv_folder_not_found');
});

test('omsvFolderDelete：有子資料夾拒刪（omsv_folder_not_empty）；子資料夾刪光後可刪', () => {
  const db = openDb(':memory:');
  const owner = 'delete-with-children@x.com';
  const parent = archive.omsvFolderCreate(db, owner, { name: '有子層的資料夾' });
  const child = archive.omsvFolderCreate(db, owner, { name: '子資料夾', parentId: parent.id });

  const delWithChild = archive.omsvFolderDelete(db, owner, { folderId: parent.id });
  assert.equal(delWithChild.error, 'omsv_folder_not_empty');

  assert.equal(archive.omsvFolderDelete(db, owner, { folderId: child.id }).ok, true);
  assert.equal(archive.omsvFolderDelete(db, owner, { folderId: parent.id }).ok, true);
});
...
test('omsv* action：未登入（無 token）→ Session expired', async () => {
  ...
  const actions = [
    ...
    { action: 'omsvFolderMove', extra: { folderId: 1, parentId: null } },
