...
// ══════════════ v213：每列軟刪除（deleted/deletedAt/deletedBy）══════════════

test('ftSaveRows：deleted 旗標往返——標 deleted:true 的既有列原樣保存（不物理移除），並記 deletedAt/deletedBy', async () => {
  const db = openDb(':memory:');
  const tok = await setupFtUser(db, 'ft-del@x.com');
  await handleRequest(db, testConfig(), { action: 'ftCreateSemester', sessionToken: tok, rootFolderId: ROOT, id: '114-1' });
  const r1 = await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ cells: { stu_id: 'B99999999', name_zh: '測試員甲' } }],
  });
  const rowId = r1.data.count === 1 ? (await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  })).data.rows[0]._id : null;
  assert.ok(rowId);

  const r2 = await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ _id: rowId, cells: { stu_id: 'B99999999', name_zh: '測試員甲' }, deleted: true }],
  });
  assert.equal(r2.success, true);
  assert.equal(r2.data.count, 1); // 未物理移除
  assert.equal(r2.data.deletedCount, 1);

  const sheet = await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  });
  assert.equal(sheet.data.rows.length, 1); // 資料檔仍保留該列（軟刪除）
  const deletedRow = sheet.data.rows[0];
  assert.equal(deletedRow.deleted, true);
  assert.ok(deletedRow.deletedAt);
  assert.equal(deletedRow.deletedBy, 'ft-del@x.com');
  assert.equal(deletedRow.cells.stu_id, 'B99999999'); // cells 內容不受刪除旗標影響，原樣保留
});

test('ftSaveRows：deleted 旗標重複儲存不覆蓋原本的 deletedAt/deletedBy（同一次刪除只記一次）', async () => {
  const db = openDb(':memory:');
  const tok = await setupFtUser(db, 'ft-del2@x.com');
  await handleRequest(db, testConfig(), { action: 'ftCreateSemester', sessionToken: tok, rootFolderId: ROOT, id: '114-1' });
  await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ cells: { stu_id: 'B99999999', name_zh: '測試員甲' } }],
  });
  const rowId = (await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  })).data.rows[0]._id;
  ...
  const firstDeletedAt = (await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  })).data.rows[0].deletedAt;

  // 另一位有權限使用者再次存檔（例如僅改別的欄位），deleted 仍為 true
  const tok2 = await setupFtUser(db, 'ft-del3@x.com');
  await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok2, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ _id: rowId, cells: { stu_id: 'B99999999', name_zh: '測試員甲' }, deleted: true }],
  });
  const sheet = await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  });
  assert.equal(sheet.data.rows[0].deletedAt, firstDeletedAt); // 未被第二次儲存覆蓋
  assert.equal(sheet.data.rows[0].deletedBy, 'ft-del2@x.com'); // 仍是第一次刪除者
});

test('ftSaveRows：deleted:false（或未帶）→ deletedAt/deletedBy 為 null', async () => {
  const db = openDb(':memory:');
  const tok = await setupFtUser(db, 'ft-del4@x.com');
  await handleRequest(db, testConfig(), { action: 'ftCreateSemester', sessionToken: tok, rootFolderId: ROOT, id: '114-1' });
  await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ cells: { stu_id: 'B99999999', name_zh: '測試員甲' } }],
  ...
  assert.equal(sheet.data.rows[0].deleted, false);
  assert.equal(sheet.data.rows[0].deletedAt, null);
  assert.equal(sheet.data.rows[0].deletedBy, null);
});

test('ftSaveRows：audit_log 記 deletedCount 與 deletedIds（僅系統配發 id，不含學號/姓名）', async () => {
  const db = openDb(':memory:');
  const tok = await setupFtUser(db, 'ft-del5@x.com');
  await handleRequest(db, testConfig(), { action: 'ftCreateSemester', sessionToken: tok, rootFolderId: ROOT, id: '114-1' });
  await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ cells: { stu_id: 'B99999999', name_zh: '測試員甲（機密不該進稽核）' } }],
  ...
  await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ _id: rowId, cells: { stu_id: 'B99999999', name_zh: '測試員甲（機密不該進稽核）' }, deleted: true }],
  });
  const auditRow = db.prepare("SELECT * FROM audit_log WHERE action = 'ftSaveRows' ORDER BY id DESC LIMIT 1").get();
  assert.match(auditRow.detail, /deletedCount=1/);
  assert.match(auditRow.detail, new RegExp(`deletedIds=${rowId}`));
  assert.doesNotMatch(auditRow.detail, /測試員甲/);
  assert.doesNotMatch(auditRow.detail, /B99999999/);
});
