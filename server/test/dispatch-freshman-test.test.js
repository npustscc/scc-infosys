...
// ══════════════ v216：每列軟刪除改為「儲存即物理移除」══════════════

test('ftSaveRows：標 deleted:true 的既有列儲存後從資料檔物理移除', async () => {
  const db = openDb(':memory:');
  ...
  assert.equal(r2.success, true);
  assert.equal(r2.data.count, 0); // 該列已從最終寫入的資料中移除
  assert.equal(r2.data.deletedCount, 1); // 本次存檔標記刪除的筆數仍照算

  const sheet = await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  });
  assert.equal(sheet.data.rows.length, 0); // 資料檔裡已經找不到這一列（物理移除，不再是軟刪除）
});

test('ftSaveRows：對已不存在的 _id 再次送出 deleted:true 是安全的 no-op（不會出錯，也不會憑空生出資料列）', async () => {
  const db = openDb(':memory:');
  ...
  // 第二次對同一個（已不存在的）_id 再送 deleted:true，不應報錯，也不應讓該列復活
  const tok2 = await setupFtUser(db, 'ft-del3@x.com');
  const r3 = await handleRequest(db, testConfig(), {
    action: 'ftSaveRows', sessionToken: tok2, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
    rows: [{ _id: rowId, cells: { stu_id: 'B99999999', name_zh: '測試員甲' }, deleted: true }],
  });
  assert.equal(r3.success, true);
  assert.equal(r3.data.count, 0);

  const sheet = await handleRequest(db, testConfig(), {
    action: 'ftGetSheet', sessionToken: tok, rootFolderId: ROOT, semester: '114-1', sheet: 'students',
  });
  assert.equal(sheet.data.rows.length, 0);
});
