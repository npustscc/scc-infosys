...
test('attendanceCommit：新增打卡→回傳合併後 records，並落 audit 紀錄（mailSent=false）', () => {
  const db = freshDb();
  const r = commit.attendanceCommit(db, {
...
test('attendanceCommit：2026-07-09 事故回歸——兩人先後打卡不互相蓋掉（不整檔覆寫）', () => {
  const db = freshDb();
  commit.attendanceCommit(db, { upserts: [{ id: 'P1', type: 'punch', email: 'a@x.com', date: '2026-07-15', timestamp: '2026-07-15T01:00:00.000Z' }] }, CTX);
  const r2 = commit.attendanceCommit(db, { upserts: [{ id: 'P2', type: 'punch', email: 'b@x.com', date: '2026-07-15', timestamp: '2026-07-15T01:05:00.000Z' }] }, CTX);
  assert.equal(r2.count, 2);
  assert.deepEqual(r2.records.map((r) => r.id).sort(), ['P1', 'P2']);
});

test('attendanceCommit：既有 id 改寫（如「更新定位」）不視為新增打卡，不觸發 punchMail 稽核', () => {
  const db = freshDb();
  commit.attendanceCommit(db, { upserts: [{ id: 'P1', type: 'punch', email: 'a@x.com', date: '2026-07-15', timestamp: '2026-07-15T01:00:00.000Z' }] }, CTX);
  commit.attendanceCommit(db, { upserts: [{ id: 'P1', type: 'punch', email: 'a@x.com', date: '2026-07-15', timestamp: '2026-07-15T01:00:00.000Z', lat: 22.6, lng: 120.6 }] }, CTX);
  const rows = db.prepare("SELECT * FROM audit_log WHERE action = 'attendanceCommit.punchMail'").all();
  assert.equal(rows.length, 1); // 只有第一次（新增）觸發，第二次（改寫既有 id）不觸發
});

test('attendanceCommit：removes 依 id 移除', () => {
  const db = freshDb();
  commit.attendanceCommit(db, { upserts: [{ id: 'P1', type: 'punch', email: 'a@x.com', date: '2026-07-15' }, { id: 'P2', type: 'punch', email: 'b@x.com', date: '2026-07-15' }] }, CTX);
  const r = commit.attendanceCommit(db, { upserts: [], removes: ['P1'] }, CTX);
  assert.deepEqual(r.records.map((x) => x.id), ['P2']);
});

test('attendanceCommit：fail-closed——attendance.json 內容損毀時 throw，絕不以空殼覆寫（核心回歸測試）', () => {
  const db = freshDb();
...
test('併發：attendanceCommit 兩人先後打卡，第二個交易看到第一個交易已提交的紀錄（RMW 非舊快照覆寫）', () => {
  const db = freshDb();
  commit.attendanceCommit(db, { upserts: [{ id: 'P1', type: 'punch', email: 'a@x.com', date: '2026-07-15' }] }, CTX);
  const r2 = commit.attendanceCommit(db, { upserts: [{ id: 'P2', type: 'punch', email: 'b@x.com', date: '2026-07-15' }] }, CTX);
  assert.equal(r2.count, 2);
});
