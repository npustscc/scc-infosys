...
test('_ftComputeMergedRows：v216 學生學號尾隨空白不影響跨表比對，且合併後顯示值已去除頭尾空白', () => {
  const S = loadMerged();
  // 學生名冊的學號帶尾隨半形與全形空白（舊資料常見殘留），測驗資料表的學號本身乾淨無空白。
  const studentsRows = [{ cells: { stu_id: 'B99999999 　', name_zh: '甲' } }];
  const testsRows = [{ cells: { stu_id: 'B99999999', name_zh: '甲', validity: '80', alpr: '96' } }];
  const { rows } = S._ftComputeMergedRows(studentsRows, testsRows, []);
  assert.equal(rows.length, 1);
  assert.equal(rows[0]._hasTest, true); // JOIN 本身早已雙邊 trim，跨表比對本來就成功
  assert.equal(rows[0].cells.stu_id, 'B99999999'); // 合併後顯示值不帶任何頭尾空白（v216 修正點）
});
