...
// 一學生一案號（個案架構重構 Slice 1）：同學期／舊學期同學生兩分支合併為資訊提示＋「帶入既有案號」，
// 不再提供「另外開案（產生新案號）」。
test('checkCaseIdDuplicate：同學期同學生（學號相符）→ 資訊提示＋帶入既有案號', () => {
  const { S, doc } = loadDup(
  ...
  assert.ok(msg.innerHTML.includes('已有案號'));
  assert.ok(msg.innerHTML.includes('察看個案資料與過去紀錄')); // #9：改名（原「查看現有案號」），並移除「帶入既有案號」按鈕
  assert.ok(!msg.innerHTML.includes('帶入既有案號'));
...
test('checkCaseIdDuplicate：姓名單一欄位相符也視為同一學生', () => {
  ...
  S.checkCaseIdDuplicate();
  assert.ok(doc._els['nc-id-message'].innerHTML.includes('已有案號'));
});

test('checkCaseIdDuplicate：舊學期同學生 → 資訊提示＋帶入既有案號（同一案號沿用，非產生新案號）', () => {
  const { S, doc } = loadDup(
  ...
  assert.equal(msg.style.display, '');
  assert.ok(msg.innerHTML.includes('已有案號'));
  assert.ok(msg.innerHTML.includes('察看個案資料與過去紀錄')); // #9
  assert.ok(!msg.innerHTML.includes('帶入既有案號'));
