...
// ══════ v177：實習生晚班班表——逐人晚班日（第三參數 evening）══════
const CFG177 = {
  weeklyHours: { 1: { start: 8, workEnd: 17, end: 18 }, 2: { start: 8, workEnd: 17, end: 18 } },
  extraWorkDays: [], semesterPeriods: [], nonSemesterEndHour: 18, holidays: [],
};

test('v177：evening={days:[1],start:12,workEnd:21}，週一（晚班日）整天請假 → 8（12:00-21:00 扣午休 16:00-17:00）', () => {
  const S = loadFns();
  const evening = { days: [1], start: 12, workEnd: 21 };
  assert.equal(S._leaveWorkHours(CFG177, '2026-07-13', '12:00', '2026-07-13', '21:00', evening), 8);
});

test('v177：同一使用者週二（非晚班日）整天請假 → 照一般 08:00-17:00 = 8（晚班設定不影響非晚班日）', () => {
  const S = loadFns();
  const evening = { days: [1], start: 12, workEnd: 21 };
  assert.equal(S._leaveWorkHours(CFG177, '2026-07-14', '08:00', '2026-07-14', '17:00', evening), 8);
});

test('v177：有 semesterPeriods 且日期在區間外 → 晚班日自動失效，以一般時段計（結果同無 evening）', () => {
  const S = loadFns();
  const cfg = { ...CFG177, semesterPeriods: [{ from: '2026-09-01', to: '2027-01-31' }] }; // 2026-07-13 在區間外
  const evening = { days: [1], start: 12, workEnd: 21 };
  const withEvening = S._leaveWorkHours(cfg, '2026-07-13', '08:00', '2026-07-13', '17:00', evening);
  const withoutEvening = S._leaveWorkHours(cfg, '2026-07-13', '08:00', '2026-07-13', '17:00');
  assert.equal(withEvening, 8);
  assert.equal(withEvening, withoutEvening);
});

test('v177：晚班日撞 holidays（假日優先＝休假）→ 0', () => {
  const S = loadFns();
  const cfg = { ...CFG177, holidays: ['2026-07-13'] };
  const evening = { days: [1], start: 12, workEnd: 21 };
  assert.equal(S._leaveWorkHours(cfg, '2026-07-13', '08:00', '2026-07-13', '21:00', evening), 0);
});

test('v177：_dayWorkStartEnd 晚班日回傳 {start:\'12:00\', end:\'21:00\'}', () => {
  const S = loadFns();
  const evening = { days: [1], start: 12, workEnd: 21 };
  assert.deepEqual(S._dayWorkStartEnd(CFG177, '2026-07-13', evening), { start: '12:00', end: '21:00' });
});
