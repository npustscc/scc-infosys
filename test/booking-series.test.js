...
function loadNextInSeries_() {
  return load(['_bkNextInSeries']);
}
...
// ── _bkNextInSeries（v97：系列預約寫紀錄自動帶入下次會談）────────────
test('_bkNextInSeries：回傳同系列日期較晚的最早一筆', () => {
  const S = loadNextInSeries_();
  const bookings = seriesFixture();
  const booking = bookings.find(b => b.id === 'bk1'); // date 2026-07-01
  const next = S._bkNextInSeries(bookings, booking);
  assert.equal(next.id, 'bk2'); // 07-08 早於 07-15
});

test('_bkNextInSeries：已是系列最後一筆時回傳 null', () => {
  const S = loadNextInSeries_();
  const bookings = seriesFixture();
  const booking = bookings.find(b => b.id === 'bk3'); // date 2026-07-15，最晚
  assert.equal(S._bkNextInSeries(bookings, booking), null);
});

test('_bkNextInSeries：非系列成員（無 seriesId）回傳 null', () => {
  const S = loadNextInSeries_();
  const bookings = [{ id: 'solo1', date: '2026-07-10' }, { id: 'solo2', date: '2026-07-20' }];
  assert.equal(S._bkNextInSeries(bookings, bookings[0]), null);
});

test('_bkNextInSeries：不同 seriesId 不列入候選', () => {
  const S = loadNextInSeries_();
  const bookings = seriesFixture();
  const booking = bookings.find(b => b.id === 'bk1');
  const next = S._bkNextInSeries(bookings, booking);
  assert.notEqual(next?.id, 'bk-other');
});
