...
// v187：待辦頁 tab 拖曳排序範圍併入「系所歸類」（'classify'，非六分類 todo，僅共用同一份拖曳
// 順序偏好），實際呼叫端改餵 TODO_TAB_ORDER_DEFAULT（= TODO_CATEGORY_ORDER_DEFAULT + 'classify'）
// 當 defaultOrder。函式本身是泛用的（不關心 key 語意），這裡驗證帶入含 'classify' 的預設順序時
// 行為與六分類時一致，不需改寫 _normalizeTodoTabOrder 本身。
test('_normalizeTodoTabOrder：defaultOrder 含 classify（v187 tab 排序範圍擴充）時一併正規化，不需改寫函式本身', () => {
  const S = loadTodoCat(['_normalizeTodoTabOrder']);
  const TAB_ORDER_DEFAULT = [...TODO_CATEGORY_ORDER_DEFAULT, 'classify'];
  // 未儲存過：回傳含 classify 的完整預設順序
  assert.deepEqual(S._normalizeTodoTabOrder(undefined, TAB_ORDER_DEFAULT), TAB_ORDER_DEFAULT);
  // 已儲存過舊格式（六分類、缺 classify，模擬升級前帳號）：classify 依預設順序補在最後
  const savedOld = ['admin', 'leave', 'transfer', 'ml', 'case', 'draft'];
  assert.deepEqual(S._normalizeTodoTabOrder(savedOld, TAB_ORDER_DEFAULT), [...savedOld, 'classify']);
  // 使用者已把 classify 拖到其他位置：原樣保留（不會被強制拉回置尾）
  const savedCustom = ['classify', 'admin', 'leave', 'transfer', 'ml', 'case', 'draft'];
  assert.deepEqual(S._normalizeTodoTabOrder(savedCustom, TAB_ORDER_DEFAULT), savedCustom);
});
